...
class ActivoFijo(models.Model):
    ...
    responsiva_ids = fields.One2many(
        'activo.fijo.responsiva',
        'activo_id',
        string='Responsivas'
    )
    display_name = fields.Char(string='Nombre del Activo')
    image_1920 = fields.Image(string="Imagen del Activo", max_width=1920, max_height=1920, store=True, attachment=True)


    @api.model
    def create(self, vals):
        if vals.get('name', 'Nuevo') == 'Nuevo':
            categoria = self.env['activo.fijo.categoria'].browse(vals.get('categoria_id'))
            if not categoria or not categoria.codigo_prefijo:
                raise ValidationError("La categoría seleccionada no tiene un prefijo definido.")
            sequence_code = f'activo.fijo.{categoria.codigo_prefijo.lower()}'
            vals['name'] = self.env['ir.sequence'].next_by_code(sequence_code) or 'Nuevo'
