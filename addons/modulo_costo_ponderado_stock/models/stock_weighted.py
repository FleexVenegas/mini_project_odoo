...
class StockWeighted(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", string="Moneda de Última Compra", readonly=False
    )
    ...
    order_id = fields.Many2one("purchase.order", string="Orden", required=False)
...
class StockMove(models.Model):
    ...
    def _action_done(self, *args, **kwargs):
        ...
        for move in self.filtered(lambda m: m.product_id):
            ...
            es_dollar = (
                move.purchase_line_id.order_id.es_dollar
    ...
    def _update_weighted_cost(
        self, product, costo, cantidad, currency, valor_dollar, order
    ):
        ponderado = costo
        product_id = product.id
        warehouse = self.env["stock.warehouse"].search(
            [("name", "=", "ALMACEN CENTRAL")], limit=1
        )
        if not warehouse:
            raise UserError("No se encontró el almacén ALMACEN CENTRAL")
        location = warehouse.lot_stock_id
        quants = self.env["stock.quant"].search(
            [("product_id", "=", product_id), ("location_id", "child_of", location.id)]
        )
            ...
            Weighted.create(
                {
                    ...
                    "ultimo_tipo_cambio": (
                        valor_dollar if currency.name == "USD" else 1.0
                    ),
        ...
        if empate_cantidades == 0:
            ...
            # cantidad_anterior = (available_qty - cantidad) or 0.0
            cantidad_anterior = weighted.current_stock or 0.0
            ...
            #     if cantidad < (cantidad_anterior / 2):
            #         currency = self.env.ref("base.USD")
            ...
            # 🔥 PROTECCIÓN GLOBAL (clave)
            if cantidad_anterior <= 0 or total_qty <= 0:
                ponderado = costo
                total_qty = cantidad

                # respetamos moneda actual
    ...
    def _calculate_and_apply_new_price_to_pricelist(self, product, order):
        ...
        if not weighted:
            raise UserError(
                f"No se encontró costo ponderado para {product.display_name}"
            )
