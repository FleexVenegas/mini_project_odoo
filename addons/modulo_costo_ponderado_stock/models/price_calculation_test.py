...
class PriceCalculationTest(models.TransientModel):
    ...
    costo = fields.Float(string="Costo base MXN", readonly=True)
            ...
            if weighted and weighted.currency_id and weighted.currency_id.name == 'MXN':
                try:
                    ...
                    if valores_mxn and not valores_mxn.get('error'):
                        valores['resultado'] = valores_mxn.get('resultado', valores['resultado'])
                        valores['formula_utilizada'] = valores_mxn.get('formula_utilizada', valores['formula_utilizada'])
                        valores['formula_completa'] = valores_mxn.get('formula_completa', valores['formula_completa'])
    ...
    def actualizar_precio_lista(self, pricelist_id, product_id, nuevo_precio):
        try:
            ...
            if pricelist_item:
                ...
                _logger.info(f"Precio actualizado para producto {product_id} en lista {pricelist_id}: {nuevo_precio}")
                return {
                    'success': True,
                    'action': 'updated',
                ...
                _logger.info(f"Nuevo precio creado para producto {product_id} en lista {pricelist_id}: {nuevo_precio}")
                return {
                    'success': True,
                    'action': 'created',
